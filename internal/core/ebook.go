...
func GenerateEbook(deps model.Dependencies, req ProcessRequest, dstPath string) (book model.BookmarkDTO, err error) {
	...
	lastline := `<hr/><p style="text-align:center">Generated By <a href="https://github.com/go-shiori/shiori">Shiori</a> From <a href="` + book.Bookmark.URL + `">This Page</a></p>`

	// Create ebook
	ebook, err := epub.NewEpub(book.Bookmark.Title)
	if err != nil {
		return book, errors.Wrap(err, "can't create EPUB")
	}

	ebook.SetTitle(book.Bookmark.Title)
	ebook.SetAuthor(book.Bookmark.Author)
	if deps.Domains().Storage().FileExists(bookmarkThumbnailPath) {
	...
	ebook.SetDescription(book.Bookmark.Excerpt)
	_, err = ebook.AddSection(`<h1 style="text-align:center"> `+book.Bookmark.Title+` </h1>`+book.Bookmark.HTML+lastline, book.Bookmark.Title, "", "")
	if err != nil {
