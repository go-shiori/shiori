...
<template>
  <AppLayout>
        ...
        <div class="flex space-x-2">
          ...
          <div class="relative">
            <input
              type="text"
              placeholder="Search..."
              class="border border-gray-300 rounded-md px-3 py-1 focus:outline-none focus:ring-2 focus:ring-red-500"
            />
          </div>
        </div>
      </div>
    ...
    <div class="mt-6">
      <ul class="space-y-4">
        <li v-for="bookmark in bookmarks" :key="bookmark.id" class="bg-white p-4 rounded-md shadow-sm hover:shadow-md transition-shadow">
          <div class="flex justify-between">
            <a :href="bookmark.url" target="_blank" class="text-blue-600 hover:underline font-medium">{{ bookmark.title }}</a>
            <div class="flex space-x-2">
          ...
          <div class="text-gray-500 text-sm mt-1 truncate">{{ bookmark.url }}</div>
