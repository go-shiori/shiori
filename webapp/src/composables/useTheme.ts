...
export function useTheme() {
  ...
  const apply = (pref: ThemePreference) => {
    try {
      ...
      if (pref === 'system') {
        ...
        desiredTheme = 'high-contrast';
        desiredDark = true;
      }

      if (
        current.value !== pref ||
        domTheme !== desiredTheme ||
        domDark !== desiredDark
      ) {
        current.value = pref;
        setHtmlTheme(pref);
  ...
  const init = () => {
    try {
      ...
      if (!browserSupport.cssCustomProperties) {
        console.warn(
          'CSS Custom Properties not supported, theme switching may not work properly'
        );
      }
  ...
  const setTheme = async (pref: ThemePreference) => {
    ...
    try {
      ...
      success(
        `Theme changed to ${themeNames[pref]}`,
