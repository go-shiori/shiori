...
onMounted(async () => {
  ...
  const pref =
    (authStore.user?.config?.Theme as any) ||
    (localStorage.getItem('shiori-theme') as any) ||
    'system';
  apply(pref);
  init();
...
<template>
  <div
    class="min-h-screen h-full flex flex-col bg-[var(--background-color)] text-[var(--text-color)]"
  >
    <div
      v-if="isInitializing"
    ...
    >
      <div class="text-center">
        <div
          class="animate-spin rounded-full h-12 w-12 border-b-2 border-red-500 mx-auto mb-2"
        ></div>
