...
const selectedTags = computed(() => {
  return tagsStore.tags.filter(
    tag => isValidTag(tag) && selectedTagIds.value.includes(tag.id)
  );
});
...
const showCreateOption = computed(() => {
  ...
  const exactMatch = searchResults.value.some(
    tag => isValidTag(tag) && tag.name.toLowerCase() === searchTerm
  );

  return (
    !exactMatch &&
    !selectedTagIds.value.some(id => {
...
const allSuggestions = computed(() => {
  const suggestions: Array<
    { type: 'tag'; tag: Tag } | { type: 'create'; name: string }
  > = [];
...
const currentSelection = computed(() => {
  if (
    ...
    selectedIndex.value < allSuggestions.value.length
  ) {
...
const createAndAddTag = async (tagName: string) => {
  try {
    ...
    // Show error toast
    error(t('tags.toast.created_error'), t('tags.toast.created_error_message'));
  }
};
...
<template>
  <div class="relative">
    ...
    <div
      v-if="isOpen && (allSuggestions.length > 0 || isSearching)"
    ...
    >
      ...
      <div
        v-if="isSearching"
        class="w-full px-3 py-2 text-center text-gray-500 dark:text-gray-400"
      >
        <div
          class="inline-block animate-spin rounded-full h-4 w-4 border-b-2 border-red-500 mr-2"
        ></div>
        {{ t('tags.searching') }}
      </div>
      ...
      <button
        ...
        @click="
          suggestion.type === 'tag'
            ...
            : createAndAddTag(suggestion.name)
        "
        :class="[
      ...
      >
        ...
        <span
          v-else
          class="text-red-600 dark:text-red-400 font-medium flex items-center space-x-2"
        >
          <PlusIcon size="14" />
