...
func OpenMySQLDatabase(username, password, dbname string) (*MySQLDatabase, error) {
	var err error
...
func (db *MySQLDatabase) CreateBookmark(bookmark model.Bookmark) (bookmarkID int64, err error) {
	// Check URL and title
	if bookmark.URL == "" {
...
func (db *MySQLDatabase) GetBookmarks(withContent bool, indices ...string) ([]model.Bookmark, error) {
...
func (db *MySQLDatabase) DeleteBookmarks(indices ...string) (err error) {
...
func (db *MySQLDatabase) GetAccounts(keyword string, exact bool) ([]model.Account, error) {
